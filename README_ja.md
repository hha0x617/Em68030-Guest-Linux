...
[Em68030](https://github.com/hha0x617/Em68030_WinUI3Cpp) MC68030/MVME147 エミュレータ用の
Linux ゲスト側カーネルモジュール、パッチ、設定ファイルです。
...
Em68030 固有の MVME147 サポート用カーネルソースパッチです。
...
Linux 6.12.17 カーネル用の保存済み `.config` ファイルです。`mvme16x_defconfig` をベースに
Em68030 固有のオプションを有効化しています。`menuconfig` を手動で実行する代わりに、
カーネルソースツリーに `.config` としてコピーできます。
